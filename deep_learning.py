...
# SPECIFICATION: Classifies fashion items
...
for layerCount in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for neuronCount in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for rate in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            #model = build_model()
            #-->add your Pyhton code here
            model = build_model(layerCount, neuronCount, len(class_names), rate)
            ...
            correctCount = 0
            class_predicted = np.argmax(model.predict(X_test), axis=-1)
            for predict, ground in zip(class_predicted, y_test):
                correctCount += predict == ground
                
            if correctCount > mostCorrect:
                highestAccuracy = correctCount / len(X_test)
                mostCorrect = correctCount
                bestModel = model
                print("Highest accuracy so far: " + str(highestAccuracy))
                print("Parameters: " + "Number of Hidden Layers: " + str(layerCount) + ",number of neurons: " + str(neuronCount) + ",learning rate: " + str(rate))
                print()
